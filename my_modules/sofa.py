...
def threshold_density(labels, X, umbral=0.5):
    ...
    noise_labels = np.where(new_labels == -1)[0]
    kdtree = cKDTree(X)
    for i in noise_labels:
        point = X[i]
        neighbors_indices = kdtree.query_ball_point(point, r=umbral)
        neighbors_indices.remove(i)
        if len(neighbors_indices) > 0:
            neighbors_labels = new_labels[neighbors_indices]
            neighbors_labels = neighbors_labels[neighbors_labels != -1]
            if neighbors_labels.size > 0:
                new_label = np.bincount(neighbors_labels).argmax()
                new_labels[i] = new_label
    # Stablish the noise points as 0
    new_noise_labels = np.where(new_labels == -1)[0]
    if len(new_noise_labels) > 0:
        new_labels[new_noise_labels] = 0
    return new_labels

def threshold_density_2(y_pred, X, n_neighbors=5):
...
''' Function that calculate the density of a given point conventionally
    Parameters: labels, X, umbral
    Return: labels with the new labels of the noise points'''
def threshold_conv(labels):
    new_labels = np.copy(labels)
    noise_labels = np.where(new_labels == -1)[0]
    new_labels[noise_labels] = 0
    return new_labels
